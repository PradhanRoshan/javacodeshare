...
public class A {
	...
	public List<Integer> filterList(List<Integer> list, 
		...
		if(filter.equalsIgnoreCase("even")) {
			//return the list with only even numbers in it
			for(int e : list) {
				if(e % 2 == 0)
					tempList.add(e);  //
			}
		...
		if(filter.equalsIgnoreCase("odd")) {
			//return the list with only odd numbers in it
			for(int e : list) {
				if(e % 2 == 1)
					tempList.add(e);  //
			}
	...
	public List<Employee> filterEmployeeList(List<Employee> list,
			double filterSalary, String filter) {
	...
	public double computePercentException(double marks,double total) 
	throws MarksException{
		...
		if(marks < 0)
			throw new MarksException("Marks cannot be less than 0");
